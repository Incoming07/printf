...
/*   Updated: 2019/09/23 19:21:17 by bglover          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	make_man(int man[], int por, int acc, int sign)
{
	int i;
	int j;
	int *rez;
...
void	find_numb(char bin_numb[], int acc)
{
	...
	make_man(man, make_por(por), acc, sign);
}
...
int	main(void)
{
	...
	acc = 1;
	doub = 123.0000033346456;
	ft_print_float((long double)doub, acc);
	printf("\n%.0f", doub);
	return (0);
}
